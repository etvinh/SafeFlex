 ...
 * @param: vec, pointer to right factor 3x1 vector
 * @param: result, pointer to vector that is modified to contain the matrix product of mat1 and mat2.
 * @return: none
 * mat1 and mat2 are not modified by this function.  result is modified by this function.
 */
 ...
 * @param: vec, a pointer to a 3D vector
 * @param: result, a pointer to a 3D vector that is modified to contain the product of scalar and vec
 * @return: none
 * scalar and vec are not modified by this function.  result is modified by this function.
*/
void VectorScalarMultiply(float scalar, float vec[3], float result[3]);

/**
 * VectorCrossProduct performs the cross product of two 3x1 vectors 
 * (result = A x B) and "returns" the result by modifying the third argument.
 */
void VectorCrossProduct(float A[3], float B[3], float result[3]);
