...
struct InsightsView: View {
    ...
    private func load() async {
        do {
            let week = InsightsCalculator.weekDates(containing: .now)
            let workouts = try await container.workouts.workouts(
                from: week[0],
                to: Calendar.current.date(byAdding: .day, value: 1, to: week[6])!
            )
            let computed = InsightsCalculator.report(from: workouts)
            report = computed
            try await container.workouts.persistLogs(for: computed)
        } catch {
