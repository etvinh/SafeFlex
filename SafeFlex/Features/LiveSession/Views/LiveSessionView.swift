...
struct LiveSessionView: View {
    ...
    // Targets come from the prescribed plan for the active exercise.
    private var planEntry: PlanEntry? { ExercisePlan.entry(for: appState.activeExercise) }
    private var totalReps: Int { planEntry?.repsPerSet ?? 12 }
    private var totalSets: Int { planEntry?.sets ?? 3 }
    ...
    private func countRep() {
        ...
        // Record this rep: peak flex mapped to degrees, stability averaged
        // over the rep and mapped to percent.
        romPerRep.append(SensorScale.romDegrees(fromFlex: repPeakFlex))
        let avgStability = repStabilitySamples > 0
            ? repStabilitySum / Double(repStabilitySamples)
            : 0
        stabilityPerRep.append(SensorScale.stabilityPercent(avgStability))
        repPeakFlex = 0
        repStabilitySum = 0
    ...
    private func endSession() {
        ...
        let repository = container.workouts
        Task {
            do {
                try await repository.save(workout)
            } catch {
                print("[Workout] Save failed: \(error)")
            }
        }
