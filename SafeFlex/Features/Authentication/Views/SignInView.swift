...
struct SignInView: View {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 14) {
                        ...
                        // Apple Sign In (needs the paid developer capability)
                        Button(action: { viewModel.appleSignInUnavailable() }) {
                            HStack(spacing: 8) {
    ...
    private func signIn() {
        Task {
            if let user = await viewModel.signIn(email: email, password: password) {
                onSuccess(user)
            }
        }
    }
