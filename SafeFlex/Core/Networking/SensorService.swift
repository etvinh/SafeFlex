...
/// Streams live readings from the wearable's WebSocket bridge.
/// Main-actor isolated: `flex`/`stability` drive SwiftUI directly.
@MainActor
@Observable
class SensorService {
    var flex: Double = 0
    ...
    var flexPercent: Int {
        SensorScale.flexPercent(flex)
    }

    var stabilityPercent: Int {
        Int(SensorScale.stabilityPercent(stability))
    }
